...
const mockRefunds: RefundRequest[] = Array.from({ length: 30 }, (_, i) => ({
  ...
  status: i % 3 === 2 ? "Rejected" : i % 3 === 1 ? "Approved" : "Pending", // some rejected, some approved, some pending
}));

export default function RejectedRefunds() {
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // Filter only rejected refunds + search
  ...
  return (
    <div>
      <PageMeta title="Rejected Refunds" description="View all rejected refund requests" />
      <PageBreadcrumb pageTitle="Rejected Refunds" />

      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
          <div>
            <h2 className="text-2xl font-semibold dark:text-white">Rejected Refunds</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Review all refund requests that have been rejected
            </p>
          </div>
                ...
                paginatedData.map((r, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      <span className="px-2 py-1 rounded-lg bg-red-600 text-white text-sm">
                        {r.status}
                      </span>
                    </td>
                  </tr>
