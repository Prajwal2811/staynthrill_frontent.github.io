...
export default function RejectedHotels() {
  ...
  const [hotels, setHotels] = useState<Hotel[]>(
    Array.from({ length: 30 }, (_, index) => ({
      id: index + 1,
      hotelId: `HTL${2001 + index}`,
      vendorId: `VND${701 + index}`,
      name: `Rejected Hotel ${index + 1}`,
      location: ["Mumbai", "Goa", "Delhi", "Jaipur"][index % 4],
      category: categories[index % 3],
      price: 2500 + index * 120,
      status: "rejected",
    }))
  );
  ...
  const confirmDelete = () => {
    if (deleteId !== null) {
      ...
      toast.error("Hotel deleted permanently");
      setDeleteId(null);
    }
  };
  ...
  return (
    <div>
      <PageMeta title="Rejected Hotels" description="Hotels rejected by admin" />
      <PageBreadcrumb pageTitle="Rejected Hotels" />
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Hotels rejected by admin (added by vendors)
            </p>
          </div>
            ...
            <tbody>
              {paginatedHotels.length > 0 ? (
                paginatedHotels.map((hotel, index) => (
                  <tr key={hotel.id} className="border-b dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                    ...
                    <td className="px-4 py-4">{hotel.category}</td>
                    <td className="px-4 py-4 font-semibold text-red-600">₹{hotel.price}</td>

                    <td className="px-4 py-4">
                      <span className="px-3 py-1 text-xs rounded-full bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300">
                        Rejected
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-4 text-center">
                      <div className="flex justify-center gap-4">
                        <button
                          onClick={() => navigate(`/hotels/rejected/view/${hotel.id}`)}
